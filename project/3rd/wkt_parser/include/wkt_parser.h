...
class WKTParser
{
  public:
    /**
     * 所有输入的坐标必须是归一化后在[0,
     * 1]范围的点，解析之后，坐标会被size表示的宽高进行反归一化，例如：当点坐标为"POINT(0.1, 0.2)"， size=(100,
     * 200)时，反归一化后点坐标为(0.1*100, 0.2*200)=(10, 40)
     *
     ...
     */
    bool ParsePoint(const std::string &src, cv::Point *point_ptr = nullptr);
     ...
     */
    bool empty() const { return (m_size.empty() || (m_points.empty() && m_lines.empty() && m_polygons.empty())); }
