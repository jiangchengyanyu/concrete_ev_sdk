...
typedef struct
{
    ...
    std::string uuid{"200003"};
} ModelConfig;

typedef struct
{
    bool enable;
    float alert_frame_ratio;
    float alert_time_thresh;
    float max_alert_wait_time;
} RecoderConfig;
...
struct Configuration
{
  ...
  public:
    ...
    std::vector<std::string> current_roi_orig_polygons_names; // 原始的多边形roi区域的名称（roi可能是多个）
    ...
    bool draw_non_target_rect = true;             // 是否画非告警目标
    bool draw_confidence = false;                 // 是否画置信度
    // --------------------------------- 通常需要根据需要修改 START -----------------------------------------
    // 3. 算法配置参数
    ModelConfig algo_config = {}; // 默认的算法配置
    // 4. 与报警信息相关的配置
    std::string language = "en";        // 所显示文字的默认语言
    int target_rect_line_thickness = 4; // 目标框粗细
    std::map<std::string, std::map<int, std::string>> target_rect_text_map = {{"en", {}},
                                                                              {"zh", {}}}; // 检测目标框顶部文字
    cv::Scalar target_rect_color = {0, 0, 255, 1.0f};                                      // 目标检测框颜色
    cv::Scalar non_target_rect_color = {0, 255, 0, 1.0f};                                  // 非目标框颜色
    ...
    // 5.告警记录
    RecoderConfig recoder_config{false, 0.8, 2.0, 5.0};

    // --------------------------------- 通常需要根据需要修改 END -------------------------------------------
    // 解析数值类型的配置
    template <typename T> bool CheckAndUpdateNumber(const std::string &key, T &val)
    {
        return m_config_value.isMember(key) && m_config_value[key].isNumeric() &&
               ((val = m_config_value[key].asDouble()) || true);
    }
    // 解析字符串类型配置的函数
    bool CheckAndUpdateStr(const std::string &key, std::string &val)
    {
        return m_config_value.isMember(key) && m_config_value[key].isString() &&
               (val = m_config_value[key].asString()).size();
    }
    // 解析字符串数组类型配置的函数
    ...
    // 解析字符串数组类型配置的函数
    bool CheckAndUpdateLabelStr(const std::string &key, std::map<int, std::string> &val)
    {
        if (m_config_value.isMember(key))
        {
            if (m_config_value[key].isArray())
            {
                for (int i = 0; i < m_config_value[key].size(); ++i)
                {
                    val[i] = m_config_value[key][i].asString();
            ...
            else if (m_config_value[key].isObject())
            {
                const auto &names = m_config_value[key].getMemberNames();
                for (const auto &name : names)
                {
                    val[std::atoi(name.c_str())] = m_config_value[key][name].asString();
                }
            }
        }
        return true;
    ...
    {
        return m_config_value.isMember(key) && m_config_value[key].isBool() &&
               ((val = m_config_value[key].asBool()) || true);
    }
    // 解析颜色配置的函数
    bool CheckAndUpdateColor(const std::string &key, cv::Scalar &color)
    {
        if (m_config_value.isMember(key) && m_config_value[key].isArray() &&
            m_config_value[key].size() == BGRA_CHANNEL_SIZE)
        {
    ...
    {
        if (m_config_value.isMember(key) && m_config_value[key].isArray() && m_config_value[key].size() == 2 &&
            m_config_value[key][0].isNumeric() && m_config_value[key][1].isNumeric())
        {
    ...
    void ParseAndUpdateArgs(const char *conf_str)
    {
        ...
        CheckAndUpdateNumber("nms_thresh", algo_config.nms_thresh);
        CheckAndUpdateBool("class_aware", algo_config.class_aware);
        CheckAndUpdateStr("uuid", algo_config.uuid);
        CheckAndUpdateStr("cid", cid);
        ...
        CheckAndUpdateBool("draw_non_target_rect", draw_non_target_rect);
        CheckAndUpdateBool("draw_confidence", draw_confidence);
        CheckAndUpdateLabelStr("mark_text_en", target_rect_text_map["en"]);
        CheckAndUpdateLabelStr("mark_text_zh", target_rect_text_map["zh"]);
        ...
        CheckAndUpdateColor("non_target_rect_color", non_target_rect_color);
        ...
        CheckAndUpdateBool("alert_recoder_enable", recoder_config.enable);
        CheckAndUpdateNumber("alert_frame_ratio", recoder_config.alert_frame_ratio);
        CheckAndUpdateNumber("alert_time_thresh", recoder_config.alert_time_thresh);
        CheckAndUpdateNumber("max_alert_wait_time", recoder_config.max_alert_wait_time);
        std::vector<std::string> roiStrs;
        if (m_config_value.isMember("polygon_1") && m_config_value["polygon_1"].isArray() &&
            m_config_value["polygon_1"].size())
        {
        ...
        if (m_config_value.isMember("polygon_1_name") && m_config_value["polygon_1_name"].isArray())
        {
            if (m_config_value["polygon_1_name"].size() != roiStrs.size())
            {
                SDKLOG(INFO) << "length of polygons and names dismated";
                exit(-1);
            }
            current_roi_orig_polygons_names.resize(roiStrs.size());
            for (int i = 0; i < m_config_value["polygon_1_name"].size(); ++i)
            {
                current_roi_orig_polygons_names[i] = m_config_value["polygon_1_name"][i].asString();
            }
        }
        ...
        if (!roiStrs.empty())
        {
            ...
            UpdateROIInfo(current_in_frame_size.width,
                          current_in_frame_size.height); // 根据当前输入图像帧的大小更新roi参数
        }
