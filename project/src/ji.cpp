...
JiErrorCode ji_get_version(char *pVersion)
{
    ...
    std::string strVersionInfo =
        "{\"sdk_version\":\"" + sdk_version + "\"," + "\"algo_version\":\"" + algo_version + "\"}";
    ...
    strncpy(pVersion, strVersionInfo.c_str(), std::min(int(strVersionInfo.size() + 1), MAX_VERSION_LENGTH));
    return jRet;
}

JiErrorCode ji_init(int argc, char **argv)
{
    // No EVDeploy runtime. The OM model is loaded by SampleDetector through AscendCL.
...
void *ji_create_predictor(JiPredictorType pdtype)
{
    ...
    if (detector->Init() != JISDK_RET_SUCCEED)
    {
        delete detector;
        detector = nullptr;
...
void ji_destroy_predictor(void *predictor)
{
    ...
    auto *detector = reinterpret_cast<SampleAlgorithm *>(
        predictor); // SampleAlgorithm为自定义的算法class类，需要根据情况进行类名修改！！！！
    detector->UnInit();
    delete detector;
    detector = nullptr;
}
                          ...
                          JiImageInfo **pOutFrames, unsigned int &nOutCount, JiEvent &event)
{
    ...
    auto *detector = reinterpret_cast<SampleAlgorithm *>(
        predictor); // SampleAlgorithm为自定义的算法class类，需要根据情况进行类名修改！！！！
    JiErrorCode processRet;
    // 例： 分析单张图片，输出单张图片，算法内部使用cv::Mat格式数据
    if (nInCount == 1)
    {
        cv::Mat inMat;
        // 获取到需要进行分析的图像数据,并根据runtime进行可能的数据格式转换
#if defined(WITH_ASCENDCL_RUNTIME) || defined(WITH_LYNXI_RUNTIME)
        cv::Mat yuv_mat = cv::Mat(ALIGN_UP2(pInFrames[0].nHeight) * 3 / 2, ALIGN_UP16(pInFrames[0].nWidth), CV_8UC1,
                                  pInFrames[0].pData);
        cv::cvtColor(yuv_mat, inMat, cv::COLOR_YUV2BGR_NV12);
        if (inMat.rows != pInFrames[0].nHeight || inMat.cols != pInFrames[0].nWidth)
        {
            inMat = inMat.rowRange(0, pInFrames[0].nHeight).colRange(0, pInFrames[0].nWidth).clone();
        }
#elif defined(WITH_SOPHON_RUNTIME)
        if (pInFrames[0].nWidth != pInFrames[0].nWidthStride) // from opencv mat
        {
            inMat = cv::Mat(pInFrames[0].nHeight, pInFrames[0].nWidth, CV_8UC3, pInFrames[0].pData,
                            pInFrames[0].nWidthStride);
        }
        else // from ffmpeg avframe
        {
            inMat = cv::Mat(pInFrames[0].nHeight, pInFrames[0].nWidth, CV_8UC3, pInFrames[0].pData);
        }
#else
    ...
    if (JISDK_RET_SUCCEED == processRet)
    {
...
JiErrorCode ji_update_config(void *predictor, const char *args)
{
    ...
    if (detector->UpdateConfig(args) != JISDK_RET_SUCCEED) // 调用自定义算法class类的配置update函数
        return JISDK_RET_FAILED;
 ...
 */
JiErrorCode ji_set_callback(void *predictor, JiCallBack callback) { return JISDK_RET_UNUSED; }
 ...
 * 在本demo中未实现，可具体根据项目和算法要求进行相关实现!!!!
 */
JiErrorCode ji_calc_image_asyn(void *predictor, const JiImageInfo *pInFrames, const unsigned int nInCount,
...
JiErrorCode ji_delete_face_db(void *predictor, const int faceDBId) { return JISDK_RET_UNUSED; }

JiErrorCode ji_get_face_db_info(void *predictor, const int faceDBId, char *info) { return JISDK_RET_UNUSED; }

JiErrorCode ji_face_add(void *predictor, const int faceDBId, const char *faceName, const int faceId, const char *data,
                        const int dataType, char *imagePath)
{
    return JISDK_RET_UNUSED;
}

JiErrorCode ji_face_update(void *predictor, const int faceDBId, const char *faceName, const int faceId,
                           const char *data, const int dataType, char *imagePath)
{
    return JISDK_RET_UNUSED;
}

JiErrorCode ji_face_delete(void *predictor, const int faceDBId, const int faceId) { return JISDK_RET_UNUSED; }
